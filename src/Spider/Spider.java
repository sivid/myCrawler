 ...
 */
  ...
  public void log(String entry)
  {
    ...
    //é¡å?–æ–°å¢žç?„ç?‹å?ç¢¼
   /*
	File wFile =new File("test2.txt");  // ï¿½Ø¥ß¤@ï¿½Ó¥Î¨Ó¼gï¿½É®×ªï¿½fileï¿½ï¿½ï¿½ï¿½ wFile
	
	try{
		//BufferedReader in = new BufferedReader(new FileReader(args[0]));
		//PrintWriter out = new PrintWriter(new FileWriter(args[1]));
		BufferedReader in = new BufferedReader(new StringReader(entry)); 
		StringBuilder sb =new StringBuilder(300);		
		PrintWriter out = new PrintWriter(new FileWriter(wFile,true));
	...
	*/
	...
	FileOutputStream fos=new FileOutputStream("ProvideByLBH.csv",true);
	while(true){
		int i=is.read();
		if(i==-1)
			break;
	...
	fos.close();
    	...
    	//System.out.println("?œ¨è®????"+entry+"??‚å‡º?Œ¯");
    }
